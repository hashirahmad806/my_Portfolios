      ...
      <div className="max-w-6xl mx-auto px-6 md:px-12">
        <div className="grid lg:grid-cols-2 gap-16 items-start">
          ...
          <div>
            ...
            <RevealDiv delay={0.1}>
              <h2 style={{ fontFamily: "'Clash Display','Syne',sans-serif", fontSize: 'clamp(36px,5vw,60px)', fontWeight: 600, lineHeight: 1.05, letterSpacing: '-0.03em', color: 'white', marginBottom: 28 }}>
            ...
            {[
              <>I'm <strong style={{ color: 'white' }}>Hashir Ahmad</strong>, a MERN Stack Developer with a passion for crafting modern, high-performance web applications. My journey began with curiosity about how the web works — and evolved into deep expertise across the full JavaScript ecosystem.</>,
              <>I specialize in building <strong style={{ color: 'white' }}>full-stack applications</strong> that are fast, scalable, and beautifully designed. From RESTful APIs in <strong style={{ color: 'white' }}>Node.js & Express</strong>, to responsive interfaces in <strong style={{ color: 'white' }}>React.js</strong>, to optimized <strong style={{ color: 'white' }}>MongoDB</strong> schemas — I own the entire product lifecycle.</>,
              ...
              <RevealDiv key={i} delay={0.15 + i * 0.1}>
                <p style={{ fontSize: 15, lineHeight: 1.85, color: muted, marginBottom: 18 }}>{text}</p>
              </RevealDiv>
            ))}
                ...
                {['Based in Pakistan 🇵🇰','Open to Remote 🌍','Full Stack Dev 💻','AI Enthusiast 🤖','Available Now ✅'].map(t => (
                  <span
                    ...
                    style={{
                      ...
                      color: muted, background: 'rgba(255,255,255,0.02)',
                      transition: 'all .2s', cursor: 'default',
                    }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = 'rgba(201,168,76,.3)'; e.currentTarget.style.color = gold }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(255,255,255,.06)'; e.currentTarget.style.color = muted }}
                  >
                    {t}
                  </span>
                ))}
            ...
            {cards.map((c, i) => (
              <RevealDiv key={c.title} delay={0.1 + i * 0.08}>
                ...
                >
                  ...
                  <div style={{ fontFamily: "'Clash Display','Syne',sans-serif", fontSize: 16, fontWeight: 600, color: 'white', marginBottom: 10 }}>{c.title}</div>
                  <div style={{ fontSize: 13, lineHeight: 1.7, color: dim }}>{c.desc}</div>
                </motion.div>
              </RevealDiv>
            ))}
          </div>
